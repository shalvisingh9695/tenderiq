...
export const ExtractionHealthCard = ({ extractionHealth = {} }) => {
  ...
  const fieldsExtracted = parseNum(extractionHealth.fieldsExtracted, 0);
  const requirementsDetected = parseNum(extractionHealth.requirementsDetected, 0);
  const deadlinesDetected = parseNum(extractionHealth.deadlinesDetected, 0);
  const documentsDetected = parseNum(extractionHealth.documentsDetected, 0);
  const lowConfidenceItems = parseNum(extractionHealth.lowConfidenceItems, 0);
  const ambiguousClausesCount = parseNum(extractionHealth.ambiguousClausesCount, 0);
  const overallQualityScore = parseNum(extractionHealth.overallQualityScore, 0);
