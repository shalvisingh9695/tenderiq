...
export const InformationGapsCard = ({ informationGaps = [], missingFields = [], onOpenPdf }) => {
  ...
  const combinedGaps = [
    ...
    ...missingFields.map((f) => ({
      title: `${extractText(f)} Not Detected`,
      description: `The ${extractText(f).toLowerCase()} requirement could not be confidently identified in the ingested document text.`,
      severity: 'warning',
    ...
    <div className="glass-card p-6 rounded-2xl border border-amber-200/90 bg-gradient-to-br from-amber-50/20 via-white to-amber-50/10 space-y-5 shadow-xs">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {displayGaps.map((gap, idx) => {
          const titleText = extractText(gap.title || gap.field || (typeof gap === 'string' ? gap : 'Unverified Requirement'));
          const descText = extractText(gap.description || gap.reason || (typeof gap === 'string' ? 'Details require verification with original PDF source.' : 'Verification required prior to bid submission.'));
          const pageNum = extractText(gap.page);
          const sectionName = extractText(gap.section);
                  ...
                  <button
                    ...
                    onClick={() =>
                      onOpenPdf({
                        page: pageNum,
                        section: sectionName,
                        text: extractText(gap.clauseText || descText || titleText)
                      })
                    }
