    ...
    <div className="glass-card p-6 sm:p-8 rounded-2xl border border-orange-100/90 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <RiskScoreMeter score={overallScore} level={overallLevel} />
        </div>
        ...
        <div className="lg:col-span-2 bg-slate-50/80 p-5 rounded-2xl border border-slate-100/90 space-y-4 flex flex-col justify-between">
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-800 leading-relaxed font-medium">
              {executiveSummary || `Comprehensive clause analysis indicates an overall ${overallLevel} risk profile (${overallScore}/100) across financial, legal, and operational parameters.`}
            </p>
          </div>
              ...
              <ul className="space-y-1.5 text-xs text-slate-700">
                {topRisks.slice(0, 3).map((risk, idx) => (
                  <li key={idx} className="flex items-start gap-1.5">
                    ...
                    <span className="line-clamp-2 font-medium">{risk}</span>
                  </li>
                ))}
                {topRisks.length === 0 && (
              ...
              <ul className="space-y-1.5 text-xs text-slate-700">
                {topPositiveSignals.slice(0, 3).map((pos, idx) => (
                  <li key={idx} className="flex items-start gap-1.5">
                    ...
                    <span className="line-clamp-2 font-medium">{pos}</span>
                  </li>
                ))}
