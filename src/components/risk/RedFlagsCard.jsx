        ...
        {redFlags.map((flag, idx) => (
          ...
          >
            ...
            <div className="flex items-center justify-between pt-2 border-t border-red-100 text-[11px] text-slate-500">
              <div className="flex items-center gap-2">
                {flag.section && <span className="font-medium text-slate-700">{flag.section}</span>}
                {flag.page && <span>Pg {flag.page}</span>}
              </div>
              ...
              {onOpenSource && (
                <button
                  onClick={() =>
                    onOpenSource({
                      value: `RED FLAG: ${flag.title}`,
                      sourceText: flag.sourceText || flag.explanation,
