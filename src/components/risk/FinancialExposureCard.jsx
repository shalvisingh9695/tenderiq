...
export const FinancialExposureCard = ({ financialExposure = {}, onOpenSource }) => {
  ...
  const totalCommitmentText = extractText(totalEstimatedCommitment);
  const exposureSummaryText = extractText(exposureSummary);
    ...
    <div className="glass-card p-6 rounded-2xl border border-orange-100/90 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {items.map((item) => {
          ...
          const amount = extractText(info.amount || info.value || (typeof info === 'string' ? info : 'Unstated in document'));
          const derivation = extractText(info.derivationType, 'Explicitly Stated');
          const deadline = extractText(info.paymentDeadline || info.deadline);
          ...
          return (
            ...
            >
              <div className="space-y-2">
                ...
                {deadline && (
                  <p className="text-xs text-slate-600 font-medium pt-1">
                    <span className="text-slate-400">Due:</span> {deadline}
                  </p>
                )}
              </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-[11px] text-slate-400">
                <span className="truncate max-w-[120px]">{section || 'Tender Clause'}</span>

                {onOpenSource && (info.sourceText || amount) && (
                  <button
                    onClick={() =>
                      onOpenSource({
                        value: `${item.label}: ${amount}`,
                        sourceText: extractText(info.sourceText || amount),
                        section: section,
                        page: page,
      ...
      {exposureSummaryText && (
        <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200/70 text-xs text-slate-700 font-medium flex items-start gap-2">
          ...
          <p><span className="font-bold text-slate-900">Capital Lockup Note:</span> {exposureSummaryText}</p>
        </div>
      )}
