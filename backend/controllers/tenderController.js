  ...
  try {
    ...
    try {
      ...
      const updatedTender = await TenderModel.update(targetId, {
        ...
        riskScore: autoRiskReport ? autoRiskReport.overallScore : (typeof tender.riskScore === 'number' ? tender.riskScore : 0),
        riskLevel: autoRiskReport ? (autoRiskReport.overallScore > 60 ? 'high' : autoRiskReport.overallScore > 35 ? 'medium' : 'low') : 'low',
        extractedData: {
...
export async function chatWithTender(req, res) {
  try {
    ...
    let tender = await TenderModel.findById(targetId);
    let documentText = tender?.extractedText || tender?.rawText || tender?.previewSnippet || tender?.documentText || req.body?.documentText;

    if (!documentText) {
      const all = await TenderModel.findAll();
      if (all.length > 0) {
        tender = all[0];
        documentText = tender.extractedText || tender.previewSnippet || '';
      }
    }
    ...
    const citation = result.sources && result.sources.length > 0 ? {
      docName: tender?.title ? `${tender.title}.pdf` : 'Tender Document',
      clause: result.sources[0].section || 'Contract Specifications',
      page: result.sources[0].page || 1,
      quote: result.sources[0].text ? result.sources[0].text.substring(0, 180) + '...' : ''
    } : null;
...
export async function askQuestion(req, res) {
  try {
    const question = req.body?.question || req.body?.message || req.body?.prompt || req.body?.query;
    const tenderId = req.body?.tenderId || req.body?.id || req.params?.id || req.params?.tenderId;
    const chunks = req.body?.chunks || req.body?.documentChunks;
    ...
    if (!question || typeof question !== 'string' || question.trim().length === 0) {
      return res.status(400).json({
        success: false,
        error: 'A non-empty question or message parameter is required.'
      });
    }

    if (tenderId) {
      tender = await TenderModel.findById(tenderId);
      if (tender && !documentText && (!chunks || chunks.length === 0)) {
        documentText = tender.extractedText || tender.previewSnippet || '';
      }
    }

    if (!documentText && (!chunks || chunks.length === 0) && !tenderId) {
      const tenders = await TenderModel.findAll();
    ...
    // Fallback to first available tender if tenderId was not found in custom uploads
    if (!documentText && (!chunks || chunks.length === 0)) {
      const all = await TenderModel.findAll();
      if (all && all.length > 0) {
        tender = all[0];
        documentText = tender.extractedText || tender.previewSnippet || '';
    ...
    if ((!documentText || documentText.trim().length === 0) && (!chunks || chunks.length === 0)) {
      return res.status(400).json({
        success: false,
        error: 'No document text or chunks found to answer your question. Please provide chunks, a tenderId, or upload a document.'
      });
    }
    ...
    const result = await ChatService.answerQuestion({
      ...
      documentText,
      chunks,
      tenderId: tenderId || tender?.id || null
    });

    const citation = result.citation || (result.sources && result.sources.length > 0 ? {
      docName: tender?.title ? `${tender.title}.pdf` : (req.body?.documentTitle ? `${req.body.documentTitle}.pdf` : 'Tender Document'),
      clause: result.sources[0].section || 'Contract Specifications',
      page: result.sources[0].page || 1,
      quote: result.sources[0].text ? result.sources[0].text.substring(0, 180) + '...' : ''
    } : null);
    ...
    const cleanMessage = error.message
      ? error.message.replace(/key=[^&]+/gi, 'key=HIDDEN')
      : 'Server error while answering question.';
