 ...
 * 2. Primary supported model alias ('gemini-3.5-flash')
 ...
 * @param {string} [options.model] - Model name (defaults to GEMINI_PRIMARY_MODEL: 'gemini-3.5-flash')
...
/**
 * Generates vector embeddings for single or multiple text inputs using Gemini embedding models.
 * Retries on transient rate limits or socket hiccups.
 *
 * @param {Object} options
 * @param {string|Array<string>} options.contents - Text or array of text strings to embed
 * @param {string} [options.model] - Model name (defaults to GEMINI_EMBEDDING_MODEL)
 * @param {number} [options.maxAttempts] - Max retry attempts per candidate model
 * @returns {Promise<Array<number>|Array<Array<number>>>} Embedding vector values
 */
export async function embedContentWithRetry({
  contents,
  model = GEMINI_EMBEDDING_MODEL,
  maxAttempts = 3
}) {
  const ai = getGeminiClient();
  const modelsToTry = Array.from(new Set([model, 'text-embedding-004', 'gemini-embedding-2-preview'])).filter(Boolean);
  let lastError = null;
  ...
  for (const currentModel of modelsToTry) {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (isQuotaExhausted && modelsToTry.indexOf(currentModel) < modelsToTry.length - 1) {
          console.warn(`[Gemini Embedding] Model ${currentModel} quota reached. Trying next model...`);
          break;
        }
        ...
        const delayMs = 1000 * attempt + Math.floor(Math.random() * 400);
        await new Promise((resolve) => setTimeout(resolve, delayMs));
      }
    }
  }
  ...
  const normalized = normalizeGeminiError(lastError);
  const finalError = new Error(normalized.userMessage || 'Failed to generate embeddings with Gemini API.');
  finalError.status = normalized.status;
  finalError.code = normalized.code;
  finalError.details = normalized.details;
  finalError.originalError = lastError;
